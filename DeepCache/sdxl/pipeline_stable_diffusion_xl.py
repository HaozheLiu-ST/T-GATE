...
from .customize_attention import register_custom_forward_deep_cache
...
class StableDiffusionXLPipeline(DiffusionPipeline, FromSingleFileMixin, LoraLoaderMixin, TextualInversionLoaderMixin):
    ...
    def __call__(
        ...
        cfg_prompt_embeds = cfg_prompt_embeds.to(device)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i < interval_seq[gate_idx] and do_classifier_free_guidance:
                    latent_model_input = torch.cat([latents] * 2) 
                    added_cond_kwargs = {"text_embeds": cfg_add_text_embeds, "time_ids": cfg_add_time_ids}
                    prompt_embeds = cfg_prompt_embeds
                else:
                    latent_model_input = latents
                    added_cond_kwargs = {"text_embeds": negative_pooled_prompt_embeds, "time_ids": negative_add_time_ids}
                ...
                register_custom_forward_deep_cache(self.unet,'Attention',gate_step=interval_seq[gate_idx],cur_step=i)
                ...
                # perform guidance
                if do_classifier_free_guidance and i < interval_seq[gate_idx] :
                    noise_pred_uncond, noise_pred_text = noise_pred.chunk(2)
                    noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_text - noise_pred_uncond)

                if do_classifier_free_guidance and guidance_rescale > 0.0 and i < interval_seq[gate_idx] :
